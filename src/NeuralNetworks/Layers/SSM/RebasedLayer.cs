        ...
        for (int bi = 0; bi < batchSize; bi++)
        {
            for (int hi = 0; hi < _numHeads; hi++)
            {
                ...
                for (int t = 0; t < seqLen; t++)
                {
                    ...
                    for (int fi = 0; fi < _headDimension; fi++)
                    {
                        T phiKVal = (_lastPhiK ?? throw new InvalidOperationException("_lastPhiK has not been initialized."))[new[] { bi, t, hi, fi }];
                        ...
                        for (int di = 0; di < _headDimension; di++)
                        {
                            ...
                            states[t + 1, fi, di] = NumOps.Add(
                                states[t + 1, fi, di],
                                NumOps.Multiply(phiKVal, (_lastValue ?? throw new InvalidOperationException("_lastValue has not been initialized."))[new[] { bi, t, flatD }]));
                        }
                    }
                }
                ...
                for (int t = seqLen - 1; t >= 0; t--)
                {
                        ...
                        for (int fi = 0; fi < _headDimension; fi++)
                            numVal = NumOps.Add(numVal,
                                NumOps.Multiply(states[t + 1, fi, di],
                                    (_lastPhiQ ?? throw new InvalidOperationException("_lastPhiQ has not been initialized."))[new[] { bi, t, hi, fi }]));

                        for (int fi = 0; fi < _headDimension; fi++)
                        {
                            T phiQVal = (_lastPhiQ ?? throw new InvalidOperationException("_lastPhiQ has not been initialized."))[new[] { bi, t, hi, fi }];
                            ...
                            for (int fj = 0; fj < _headDimension; fj++)
                                numVal = NumOps.Add(numVal,
                                    NumOps.Multiply(states[t + 1, fj, di],
                                        (_lastPhiQ ?? throw new InvalidOperationException("_lastPhiQ has not been initialized."))[new[] { bi, t, hi, fj }]));
                    ...
                    for (int fi = 0; fi < _headDimension; fi++)
                    {
                        ...
                        for (int di = 0; di < _headDimension; di++)
                        {
                            ...
                            dPhiK[fi] = NumOps.Add(dPhiK[fi],
                                NumOps.Multiply(dS[fi, di], (_lastValue ?? throw new InvalidOperationException("_lastValue has not been initialized."))[new[] { bi, t, flatD }]));
                    ...
                    T dotQ = NumOps.Zero;
                    for (int d = 0; d < _headDimension; d++)
                        dotQ = NumOps.Add(dotQ, NumOps.Multiply(dPhiQ[d], (_lastPhiQ ?? throw new InvalidOperationException("_lastPhiQ has not been initialized."))[new[] { bi, t, hi, d }]));
                    ...
                    T dotK = NumOps.Zero;
                    for (int d = 0; d < _headDimension; d++)
                        dotK = NumOps.Add(dotK, NumOps.Multiply(dPhiK[d], (_lastPhiK ?? throw new InvalidOperationException("_lastPhiK has not been initialized."))[new[] { bi, t, hi, d }]));
                    ...
                    for (int d = 0; d < _headDimension; d++)
                    {
                        ...
                        T phiQd = (_lastPhiQ ?? throw new InvalidOperationException("_lastPhiQ has not been initialized."))[new[] { bi, t, hi, d }];
                        T dU_Q = NumOps.Multiply(NumOps.Subtract(dPhiQ[d], NumOps.Multiply(phiQd, dotQ)), qNormInv);

                        T phiKd = (_lastPhiK ?? throw new InvalidOperationException("_lastPhiK has not been initialized."))[new[] { bi, t, hi, d }];
